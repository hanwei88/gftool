...
func generateDaoInternal(in generateDaoInternalInput) {
	...
	modelContent := gstr.ReplaceByMap(
		...
		g.MapStrStr{
			...
			tplVarColumnNameAlias:         gstr.Trim(generateColumnNameAliasForDao(in.FieldMap, removeFieldPrefixArray)),
	...
	once.Do(func() {
		// 生成默认等级
		const DaoTxLevelTemplate = consts.TemplateGenDaoTxLevelTemplate
		var txLevelFile = filepath.FromSlash(gfile.Join(in.DirPathDaoInternal, "tx_level.go"))
		in.genItems.AppendGeneratedFilePath(txLevelFile)
		if err := gfile.PutContents(txLevelFile, strings.TrimSpace(DaoTxLevelTemplate)); err == nil {
			utils.GoFmt(txLevelFile)
			mlog.Print("generated:", txLevelFile)
		} else {
			fmt.Println("生成事务文件失败: ", err)
		}
	})
}
...
func generateColumnNameAliasForDao(fieldMap map[string]*gdb.TableField, removeFieldPrefixArray []string) string {
	var (
		buffer = bytes.NewBuffer(nil)
		array  = make([][]string, len(fieldMap))
		names  = sortFieldKeyForDao(fieldMap)
